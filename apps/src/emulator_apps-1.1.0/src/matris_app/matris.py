...
class Matris(object):
    ...
    def update(self, timepassed):
        ...
        for event in pygame.event.get():            
            ...
            if pressed(pygame.K_SPACE) or keys_accel == 'D':
                self.hard_drop()
            elif pressed(pygame.K_UP) or pressed(pygame.K_w) or keys_accel == 'S':
                self.request_rotation()
            elif pressed(pygame.K_LEFT) or pressed(pygame.K_a) or keys_accel == 'L':
                self.request_movement('left')
                self.movement_keys['left'] = 1
            elif pressed(pygame.K_RIGHT) or pressed(pygame.K_d) or keys_accel == 'R':
                self.request_movement('right')
                self.movement_keys['right'] = 1

            elif unpressed(pygame.K_LEFT) or unpressed(pygame.K_a)  or keys_accel == 'F':
                self.movement_keys['left'] = 0
                self.movement_keys_timer = (-self.movement_keys_speed)*2
            elif unpressed(pygame.K_RIGHT) or unpressed(pygame.K_d)  or keys_accel == 'F':
