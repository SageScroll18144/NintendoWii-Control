...
class Drawing:
    def __init__(self, sc, sc_map, player, clock):
        ...
        self.textures = {
            ...
            2: pygame.image.load(os.path.join(
                base_path, 'img/wall5.png')
            ).convert(),
            3: pygame.image.load(os.path.join(
                base_path, 'img/wall3.png')
            ).convert(),
            4: pygame.image.load(os.path.join(
                base_path, 'img/wall7.png')
            ).convert(),
            'S': pygame.image.load(os.path.join(
                base_path, 'img/sky0.png')
            ).convert(),
            }
        # menu
        self.menu_trigger = True
        self.menu_picture = pygame.image.load(os.path.join(
            base_path, 'img/bg.jpg'
        )).convert()
        # weapon parameters
        self.weapon_base_sprite = pygame.image.load(os.path.join(
            base_path, 'sprites/weapons/shotgun/base/0.png')
        ).convert_alpha()
        self.weapon_shot_animation = deque(
            [pygame.image.load(os.path.join(
                base_path, f'./sprites/weapons/shotgun/shot/{i}.png')
        ...
        self.shot_sound = pygame.mixer.Sound(
            os.path.join(base_path, 'sound/shotgun.wav')
        )
        # shot SFX
        self.sfx = deque(
            [pygame.image.load(
                os.path.join(base_path, f'./sprites/weapons/sfx/{i}.png')
    ...
    def menu(self):
        ...
        pygame.mixer.music.load(os.path.join(
            base_path, 'sound/win.mp3'
        ))
        pygame.mixer.music.play()
        button_font = pygame.font.Font(os.path.join(
            base_path, 'font/font.ttf'
        ), 72)
        label_font = pygame.font.Font(os.path.join(
            base_path, 'font/font1.otf'
        ), 400)
